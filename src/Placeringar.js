...
async function scaleToFit(content, setUpdateSize, updateSize) {
  ...
  alert(
    engelska
      ? "To get a correct printout, it is important that you check 'Background graphics' in the print settings."
      : 'För att få en korrekt utskrift är det viktigt att du bockar i "Bakgrundsgrafik" i utskriftsinställningarna." '
  );
  window.print();
...
const SkapaPlaceringar = () => {
    ...
    <div className="flex flex-wrap justify-center gap-4">
      ...
      >
        <div style={{ height: "46px" }}>
          <h2 className="text-xl mt-2 font-bold mr-1">
            {engelska ? "Class" : "Klass"}:
          </h2>
        </div>
        <div
          style={{
                      ...
                      >
                        <div className="flex justify-center items-center">
                          <div className="w-[155px] truncate">
                            {klassnamn ||
                              (engelska ? "Choose class..." : "Välj klass...")}
                          </div>
...

      {
        <div className="w-fit flex justify-center items-center">
                ...
                {visaKlassrumsmeny ? (
                  ...
                  >
                    <div className="flex justify-center items-center">
                      <div className="w-[155px] truncate">
                        {klassrumsnamn ||
                          (engelska ? "Choose room..." : "Välj klassrum...")}
                      </div>
                      <img
                        style={{
          ...
          <div className="relative flex items-center justify-center group ml-10">
            <img
              id="delaKnapp"
              src="/person-add.svg"
              alt="Dela"
          ...
          {isShareModalVisible && (
            <ShareModal
              ...
              buttonText={engelska ? "Share Placement" : "Dela Placering"}
            />
          )}
        </div>
      }
  ...
  useEffect(() => {
    if (nyttPlaceringsnamn) {
      document.title = "Skola77 | " + nyttPlaceringsnamn;
    } else {
      document.title = engelska ? "Seating Plan" : "Bordsplacering";
    }
  }, [engelska, nyttPlaceringsnamn]);
      ...
      {placeringsId || (data && !data.placeringar[0]) ? (
            ...
            <div
              onClick={() => {
                if (
                  ...
                  window.confirm(
                    engelska
                      ? "You have unsaved changes. Do you want to exit anyway? If not, press cancel and save first."
            ...
            <div className="text-wrap text-xl scrollbar-none overflow-x-scroll border text-center">
              <div>
                {data.klasser.length > 0 && data.klassrum.length > 0
                  ? engelska
                    ? "Start by selecting a class and a classroom"
                    : "Börja med att välja klass och klassrum "
                  : data.klassrum.length > 0
                  ? engelska
                    ? 'Go to "classes" to create a "class"'
                    : "Gå till klasser för att skapa en klass"
                  : data.klasser.length > 0
                  ? engelska
                    ? 'Go to "classrooms" to create a new classroom'
                    : "Gå till klassrum för att skapa ett klassrum"
                  : `${
                      engelska
                        ? "You need to create a class and a classroom to continue. See the menu"
                  ...
                  <input
                    ...
                    onBlur={() =>
                      nyttPlaceringsnamn ===
                        (klassnamn || "") +
                          (engelska ? " in " : " i ") +
                          (klassrumsnamn || "") || nyttPlaceringsnamn === ""
            ...
            <div
              onClick={() => {
                if (
                  window.confirm(
                    engelska
                      ? "Are you sure that you want to delete the seating plan. If not, press cancel"
                  ...
                  const formData = new FormData();
                  formData.append("id", placeringsId);

                  fetch("http://localhost:5051/user/delete-seatingplan", {
                    credentials: "include",
                    method: "POST",
                    body: formData,
                  })
        ...
        <div className="text-center">
          {data && data.placeringar.length > 0 && (
            <div>
              <h2 className="text-3xl mt-2">
                {engelska ? "Your seating plans" : "Dina placeringar"}
              </h2>
                  ...
                  data.placeringar.map((placering) => {
                    return (
                      ...
                      >
                        {placering.namn ||
                          (placering.klass.namn ||
                            (engelska ? "an empty class" : "en tom klass")) +
                            " i " +
                            (placering.klassrum.namn ||
                              (engelska
                                ? "an empty classroom"
          ...
          <ul className="overflow-y-hidden m-auto  w-[98vw] h-full border border-black mt-2">
            ...
            >
              {engelska
                ? "Press here to create a new seating plan..."
          ...
          <div className="" ref={content} style={{ zIndex: "100" }}>
            <div className="my-3">
              <div
                className={
                  vägg &&
                  "m-auto p-5 w-fit fit-content rounded-lg border-black border-8"
                }
              >
                <Klassrum
                  extra={
                    ...
                    !data.placeringar.some(
                      ...
                      >
                        {engelska
                          ? "Psst... the student names appear at the bottom of the page."
